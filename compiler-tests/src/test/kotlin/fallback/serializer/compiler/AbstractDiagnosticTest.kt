...
// inline markup and the neighbouring .diag.txt file. Other Kotlin versions format the .diag.txt
// differently, so only the pinned one checks it.
private val isFullTextChecked = System.getProperty("fallback.checkFullDiagnostics").toBoolean()
  ...
  override fun configure(builder: TestConfigurationBuilder): Unit =
    with(builder) {
      ...
      defaultDirectives {
        ...
        if (isFullTextChecked) +RENDER_DIAGNOSTICS_FULL_TEXT
