...
if [ "$FORCE" = true ]; then
    ...
    run_ktfmt() {
        # Compiler test data holds diagnostic markup that isn't valid Kotlin
        find . \( -type d -name build -o -path ./compiler-tests/src/test/data \) -prune -o -type f \( -name "*.kt" -o -name "*.kts" \) -print0 | xargs -0 "$@"
    }
else
    BASE_BRANCH="main"
    ...
    CHANGED_FILES=$(git_changed_files "$MERGE_BASE" | grep -E '\.kts?$' | grep -v '^compiler-tests/src/test/data/' || true)
